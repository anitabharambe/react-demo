...
class SpecialsScreen extends Component {
  
  static navigationOptions = ({ navigation }) => ({
    title: "Specials",
    drawerIcon: ({ tintColor }) => {
      return(
        <FontAwesomeIcon name="video-camera"  size={25} style={{ color: tintColor}}  />
      );      
    },
    headerTitleStyle: {
      color: '#fff'
  ...
  componentWillMount(){
    this.content = customData.specials;
  }
        ...
        <View style={{flexDirection: 'row', flex: 1, backgroundColor: '#414a4c'}}>
          ...
          <View style={{flex: 1}}>
            <View style={{flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
              <Text style={ styles.headingName } >Specials</Text>
            </View>
