...
class ShowsScreen extends Component {
  ...
  static navigationOptions = ({ navigation }) => ({
    //drawerIcon: <FeatherIcon name="tv"  size={25} style={{ tintColor: 'yellow '}}  />
    drawerIcon: ({ tintColor }) => {
      return(
        <FeatherIcon name="tv"  size={25} style={{ color: tintColor}}  />
      );      
    },
    headerTitleStyle: {
      color: '#fff'
        ...
        <View style={{flexDirection: 'row', flex: 1, backgroundColor: '#414a4c'}}>
          ...
          <View style={{flex: 1}}>
            <View style={{flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
              <Text style={ styles.headingName } >Shows</Text>
            </View>
